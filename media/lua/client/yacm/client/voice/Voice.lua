...
local function GetSoundPathFromPhoneme(phoneme, isFirstletter)
    local filePhoneme
    if phoneme == 'K' or phoneme == 'Q' then
        filePhoneme = 'C'
    elseif phoneme == 'Y' and isFirstletter then
        filePhoneme = 'YStart'
    else
        filePhoneme = phoneme
    end

    return 'Voice1' .. filePhoneme
end

function Voice:subscribe()
    if self.event then
        return
    end
    self.event = function()
...
function Voice:createSoundTable()
    local soundTable = {}
    local size = 0
    local index = 1
    ...
    while index <= messageSize do
        ...
        elseif isPunctuationMark(firstLetter) then
            ...
            if phoneme == nil then
                ...
                soundFile = GetSoundPathFromPhoneme(phoneme, self.firstletter)
...
function Voice:new(message, object)
    Voice.__index = self
    local o = {}
    setmetatable(o, self)
    ...
    o.soundTable, o.soundTableSize = o:createSoundTable()
